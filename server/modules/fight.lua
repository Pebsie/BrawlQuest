          ...
          for k = 1, #playersInThisFight do --cycle through plkayers
            ...
            if tostring(atkInfo) == "true" then
              if distanceFrom(pl[thisPlayer].x+16, pl[thisPlayer].y+16, mob.x[v]+(mb.img[mob[v]]/2), mob.y[v]+(mb.img[mob[v]]/2)) < mb.img[mob[v]] and not mb.friend[mob[v]] then
                local pdmg = 0
                if pl[thisPlayer].wep and item[pl[thisPlayer].wep] and love.math.random(1,10) == 1 then --critical hit!
                  pdmg = love.math.random(item[pl[thisPlayer].wep].val,item[pl[thisPlayer].wep].val*(pl[thisPlayer].str+1))
                elseif  pl[thisPlayer].wep and item[pl[thisPlayer].wep] then
                  pdmg = love.math.random(item[pl[thisPlayer].wep].val/2,item[pl[thisPlayer].wep].val + pl[thisPlayer].str)
                end
                mob.hp[v] = mob.hp[v] - pdmg
                pl[thisPlayer].score = pl[thisPlayer].score + (pdmg*(round(pl[thisPlayer].combo)+1))/item[pl[thisPlayer].wep].val
          ...
          --    if love.math.random(1,250) == 1 then inflictAspect(thisPlayer,"Bleeding") end
            ...
            elseif pl[thisPlayer].spell == "Slam" then
              if distanceFrom(pl[thisPlayer].x+16, pl[thisPlayer].y+16, mob.x[v]+(mb.img[mob[v]]/2), mob.y[v]+(mb.img[mob[v]]/2)) < 32*5 then
                mob.hp[v] = mob.hp[v] - ((pl[thisPlayer].int+item[pl[thisPlayer].wep].val)*16)*dt
              end
